...
import { List, Typography, Button, Tooltip, Space, Divider, message } from 'antd'
import { UserOutlined, GithubOutlined, MailOutlined, BugOutlined, EnvironmentOutlined } from '@ant-design/icons'
import { map, filter, sum, sortBy, flatten, intersection, groupBy, uniq } from 'lodash'
import { inflateRaw } from 'pako/dist/pako_inflate.js'
import { isEmptyArray, copyText } from '@nbfe/tools'
import { name, homepage, author, bugs } from '../package.json'
...
export const ThemeKey = 'eslint-formatter-html-theme'

export { EslintCwd, EslintCreateTime }
...
export const SwitchDarkMode = (darkMode, updateStorage) => {
  const darkKey = darkMode ? 'dark' : 'light'
  document.body.dataset.theme = darkKey
  if (updateStorage) {
    window.localStorage.setItem(ThemeKey, darkKey)
  }
}

export const renderPkgInfo = () => {
  const infoList = [
    {
      icon: <GithubOutlined />,
      href: homepage,
