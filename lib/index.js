...
const { Moon, Sun, DownOutlined, RightOutlined, VerticalAlignTopOutlined } = Icons
      ...
      <Card
        title={
          <>
            ...
            {AllFilesCount ? <Tag key="all">{AllFilesCount}</Tag> : null}
            {ErrorFilesCount ? (
              <Tag color="error" key="error">
                {ErrorFilesCount}
              </Tag>
            ) : null}
            {WarningFilesCount ? (
        ...
        extra={
          <>
            ...
            <Tooltip title="toggle open/fold all" placement="topRight" key="Tooltip">
              <Button
      ...
      {Boolean(SuccessFileCount) && (
        <Collapse defaultActiveKey={['a']} expandIconPosition="end" bordered={true}>
          <Panel
            ...
            key="a"
          >
            <Row className="success-file-path">
              {NoProblematicEslintResults.map(v => {
                const { filePath } = v
                return (
                  <Col xl={{ span: 12 }} span={24} key={filePath}>
                    <div>
